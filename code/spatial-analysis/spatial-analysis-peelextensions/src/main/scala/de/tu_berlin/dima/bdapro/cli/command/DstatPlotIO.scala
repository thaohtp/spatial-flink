...
@Service("dstat:plot:io")
class DstatPlotIO extends Command {
    ...
    try {
      ...
      val (tMin, tMax) = SQL(
        """
          ...
          |AND       ex.suite            =  {suite}
          |AND       ee.name             =  'dstat_dsk_total:read'
        """.stripMargin.trim
      ).on(
        "run" -> run,
      ...
      logger.info(s"Querying Data IO measurements for ($suite, $experiment, $run)")
      ...
      val data = {
        val stmt = conn.prepareStatement(
          """
            ...
            |AND      ex.name                   = ?
            |AND      ex.suite                  = ?
            |AND      d1.name                   = 'dstat_dsk_total:read'
            |AND      d2.name                   = 'dstat_dsk_total:writ'
            |ORDER BY host, time;
          """.stripMargin
        )
      ...
      val dataPath = basePath.resolve(s"$suite-$experiment-$run-io_performance.dat")

      // ensure base folder exists
      shell.ensureFolderIsWritable(basePath)

      // write data file
      logger.info(s"Writing Total IO performance results into path '$dataPath'")
      ...
      for {
        ...
        // Saving data to disk
        for (((host, time, read, write), i) <- data.zipWithIndex) {
          //@formatter:off
          pwriter.println(Seq(
            s"$host"                            padTo(30, ' '),
            f"${(time - tMin)/1000.0}%1.3f"     padTo(10, ' '),
            f"$read%1.3f"                       padTo(15, ' '),
            f"$write%1.3f"                      padTo(15, ' ')
          ) mkString "")
          //@formatter:on
        }
      }
      ...
      logger.info(s"Plotting IO performance results using Gnuplot")
      ...
      shell !(
        cmd =
          raw"""
             ...
             |        -e "run='$run'" \
             |        -e "hosts='$hosts'" \
             |        $gplsPath/DstatPlotIO.gpl
             |        """.stripMargin.trim,
        errorMsg =
          "Cannot plot 'DstatPlotIO.gpl' with Gnuplot")
    }
    catch {
      case e: Throwable =>
        logger.error(s"Error while plotting IO performance results for suite '$suite'".red)
        throw e
    } finally {
